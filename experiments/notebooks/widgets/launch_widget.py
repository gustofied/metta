...
class LaunchWidget(ReactiveWidget):
    ...
    def _create_layout(self):
        ...
        advanced_inputs = widgets.VBox([self.wandb_tags, self.additional_args])
    ...
    def _on_launch_click(self, button):
        ...
        with self.status_output:
            ...
            # Parse tags
            tags = [
                tag.strip() for tag in self.wandb_tags.value.split(",") if tag.strip()
            ]
            ...
            print(
                f"Config: {self.gpus.value} GPUs, {self.nodes.value} nodes, {'spot' if self.spot.value else 'on-demand'}"
            )
            ...
            if success:
                ...
                print("❌ Failed to launch training run")
...
class MultiLaunchWidget(LaunchWidget):
    ...
    def __init__(self, state: "RunState", **kwargs):
        ...
        self.vary_seeds = widgets.Checkbox(
            value=True, description="Vary Seeds", style={"description_width": "initial"}
        )
    ...
    def _on_launch_click(self, button):
        ...
        with self.status_output:
            ...
            for i in range(self.num_runs.value):
                run_name = (
                    f"{self.run_name.value}.{i + 1}"
                    if self.num_runs.value > 1
                    else self.run_name.value
                )
...
def generate_cells(
    ...
    # Section header
    cells.append(
        {"cell_type": "markdown", "metadata": {}, "source": "## 🚀 Launch Training"}
    )
...
launch_widget.display()""",
    ...
    cells.append(
        {"cell_type": "markdown", "metadata": {}, "source": "### Launch Multiple Runs"}
    )
