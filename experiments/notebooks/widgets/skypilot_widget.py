...
from experiments.notebooks.widget import ReactiveWidget, RefreshButton
...
class SkypilotWidget(ReactiveWidget):
    ...
    def _cancel_selected(self, button) -> None:
        ...
        print(f"\nCancelled {cancelled_count}/{len(self.selected_jobs)} jobs")
    ...
    def _launch_all(self, button) -> None:
        ...
        print(f"Launching {len(self.state.training_job_configs)} training jobs...")
        ...
        for i, config in enumerate(self.state.training_job_configs):
            ...
            if job.launch():
                ...
                print(f"❌ Failed to launch {job_name}")
        
        print(f"\n✅ Launched {launched_count}/{len(self.state.training_job_configs)} jobs")
        self.update()
    
    def _kill_all(self, button) -> None:
        """Kill all running jobs."""
        running_jobs = [j for j in self.state.training_jobs if j.job_id and not j.cancelled]
        ...
        for job in running_jobs:
            try:
                if self.state.service.cancel_job(job.job_id):
                    killed_count += 1
                    job.cancelled = True
                    print(f"✅ Killed {job.name} (Job ID: {job.job_id})")
                else:
                    print(f"❌ Failed to kill {job.name}")
            except Exception as e:
                print(f"❌ Error killing {job.name}: {e}")
        
        print(f"\n✅ Killed {killed_count}/{len(running_jobs)} jobs")
        self.update()
    ...
    def render(self) -> None:
        ...
        preloaded_jobs = sum(1 for job in self.state.training_jobs if hasattr(job, 'notes') and 'Pre-loaded' in str(job.notes))
        ...
        if preloaded_jobs > 0:
            badges.append(f'<span class="job-count-badge" style="background: #6c757d;">{preloaded_jobs} from experiment</span>')
        if session_jobs > 0:
            badges.append(f'<span class="job-count-badge" style="background: #28a745;">{session_jobs} this session</span>')
        
        badges_html = ' '.join(badges) if badges else '<span class="job-count-badge">0 active</span>'
        ...
        controls = widgets.VBox([
            widgets.HBox([
                self.toggle_btn,
                self.refresh_btn,
                widgets.HTML(f'<span style="margin-left: 20px; color: #6c757d;">Last updated: {datetime.now().strftime("%H:%M:%S")}</span>')
            ]),
            widgets.HBox([
                self.launch_all_btn,
                self.cancel_btn,
    ...
    def _render_job_with_checkbox(self, job: TrainingJob) -> widgets.HBox:
        ...
        checkbox.observe(lambda change: self._on_job_selection_change(change, job.job_id), 'value')
        ...
        return widgets.HBox([checkbox, job_display], layout=widgets.Layout(align_items='center'))
    ...
    def _render_session_jobs(self) -> None:
        ...
        if self.state.training_jobs:
            # Group jobs by type
            preloaded = [j for j in self.state.training_jobs if hasattr(j, 'notes') and 'Pre-loaded' in str(j.notes)]
            session = [j for j in self.state.training_jobs if not (hasattr(j, 'notes') and 'Pre-loaded' in str(j.notes))]
            
            # Display preloaded jobs if any
            if preloaded:
                display(widgets.HTML('<h4 style="margin: 15px 0 10px 0; color: #495057;">📦 Experiment Jobs (Pre-loaded)</h4>'))
            ...
            if session:
                ...
                display(widgets.HTML('<h4 style="margin: 15px 0 10px 0; color: #495057;">🌟 Current Session Jobs</h4>'))
                for job in session:
                    display(self._render_job_with_checkbox(job))
    ...
    def _render_all_sky_jobs(self) -> None:
        ...
        if sky_df.empty:
            display(widgets.HTML('<div class="status-empty"><h3>No Sky jobs found</h3></div>'))
            return
        
        # Get job IDs by type
        all_tracked_ids = [j.job_id for j in self.state.training_jobs if j.job_id]
        preloaded_ids = [j.job_id for j in self.state.training_jobs if j.job_id and hasattr(j, 'notes') and 'Pre-loaded' in str(j.notes)]
        session_ids = [j.job_id for j in self.state.training_jobs if j.job_id and not (hasattr(j, 'notes') and 'Pre-loaded' in str(j.notes))]
        ...
        for header in headers:
            html_parts.append(f'<th style="padding: 12px; text-align: left; font-weight: 600;">{header}</th>')
        ...
        for _, row in sky_df.iterrows():
            ...
            html_parts.append(f'<tr style="border-bottom: 1px solid #e9ecef; {row_style}">')
            
            # Job ID with indicator
            job_id = job_id_str
            if is_session:
                job_id = f'<span style="color: #155724; font-weight: 600;">✓ {job_id}</span>'
            elif is_preloaded:
            ...
            # Name
            name = row.get('NAME', '-')
            html_parts.append(f'<td style="padding: 10px; font-weight: 500;">{name}</td>')
            ...
            html_parts.append(f'<td style="padding: 10px; color: {status_color}; font-weight: 600;">{status}</td>')
        ...
        if 'EST_COST_USD' in sky_df.columns:
            ...
            tracked_cost = sky_df[sky_df['ID'].isin(all_tracked_ids)]['EST_COST_USD'].sum() if all_tracked_ids else 0
            session_cost = sky_df[sky_df['ID'].isin(session_ids)]['EST_COST_USD'].sum() if session_ids else 0
...
def create_enhanced_status_widget(df: pd.DataFrame) -> Any:
    ...
    for _, row in df.iterrows():
        ...
        for col in df.columns:
            ...
            if col == 'state' and pd.notna(value):
                ...
                value = f'<a href="{value}" target="_blank" class="wandb-link">W&B →</a>'
    ...
    if 'cost' in df.columns:
        ...
        if total_cost > 0:
            html_parts.append(f'<div style="margin-top: 10px; font-size: 14px;">')
...
def generate_cells(state_widget_in_header: bool = True) -> List[Dict[str, Any]]:
    ...
    cells.append({
        ...
        "metadata": {},
        "outputs": [],
        "source": """# Training status dashboard with launch/kill controls
from experiments.notebooks.widgets.skypilot_widget import SkypilotWidget

status_widget = SkypilotWidget(state)
status_widget.display()"""
    })
    
    # Add simple refresh cell
    cells.append({
        "cell_type": "code",
        "execution_count": None,
        "metadata": {},
        "outputs": [],
        "source": """# Refresh the status display
