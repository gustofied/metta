...
class TreeHelpFormatter(argparse.HelpFormatter):
    ...
    def __init__(self, *args, collapse: bool = False, use_colors: bool = True, user_overrides: Optional[Dict[str, Any]] = None, **kwargs):
        ...
        self.use_colors = use_colors and sys.stdout.isatty()  # Only use colors in terminal
    ...
    def _format_action(self, action):
        ...
        opt_name = option_strings[0] if option_strings else '--' + action.dest.replace('_', '-')
...
def format_help_with_defaults(config_class: Type[BaseModel], 
                            prog_name: str,
                            has_positional_name: bool = True,
                            collapse: bool = False,
                            user_overrides: Optional[Dict[str, Any]] = None,
                            patched_defaults: Optional[Dict[str, Any]] = None) -> str:
    ...
    parser = argparse.ArgumentParser(
        prog=prog_name,
        formatter_class=lambda prog: TreeHelpFormatter(prog, collapse=collapse, user_overrides=user_overrides),
        add_help=False
    )
    ...
    # Add collapse option
    parser.add_argument('--help-compact', action='store_true', 
    ...
    def add_fields(model_class: Type[BaseModel], prefix: str = '', level: int = 0):
        ...
        for field_name, field_info in all_fields.items():
            ...
            if hasattr(default, '__class__') and default.__class__.__name__ == 'PydanticUndefined':
                default = None
            ...
            elif type_origin in (dict, typing.Dict) if 'typing' in locals() else (dict,):
    ...
    examples += f"  metta notebook arena my_experiment --gpus 2\n"
    examples += f"  metta notebook arena --gpus 4 --nodes 2 --launch\n"
    examples += f"  metta notebook arena test_run --output_dir=None  # Run without creating notebook\n"
...
def setup_cli_parser(config_class: Type[BaseModel], 
                    prog_name: str,
                    args_to_parse: List[str],
                    has_positional_name: bool = True) -> Dict[str, Any]:
