...
class SkypilotService:
    ...
    def _execute_command(self, cmd: List[str]) -> LaunchResult:
        ...
        try:
            ...
            if success:
                ...
                self.log.error(f"✗ Launch failed with return code: {process.returncode}")
    ...
    def cancel_job(self, job_id: str) -> bool:
        ...
        try:
            ...
            if success:
                ...
                self.log.error(f"✗ Failed to cancel job {job_id}: {result.stderr.strip()}")
    ...
    def get_sky_jobs_data(self, include_all: bool = False) -> pd.DataFrame:
        ...
        try:
            ...
            # Add cost estimation
            if not df.empty and "RESOURCES" in df.columns and "JOB DURATION" in df.columns:
                ...
                df["EST_COST"] = df["EST_COST_USD"].apply(lambda x: f"${x:.2f}" if x > 0 else "-")
    ...
    def _estimate_job_cost(self, resources: str, duration_hours: float, spot: bool = True) -> float:
    ...
    def get_wandb_run_name_from_sky_job(self, sky_job_id: str) -> Optional[str]:
        ...
        try:
            ...
            if result.returncode != 0:
                self.log.debug(f"Failed to get logs for job {sky_job_id}: {result.stderr}")
                return None
    ...
    def get_training_status(
        ...
        wandb_status = wandb_service.get_run_statuses(wandb_run_names, show_metrics, entity, project)
