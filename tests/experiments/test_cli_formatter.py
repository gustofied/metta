...
class TestFormatHelpWithDefaults:
    ...
    def test_examples_section(self):
        """Test that examples section is properly formatted."""
        help_text = format_help_with_defaults(
            TestConfig,
            prog_name="test_prog"
        )
        ...
        assert "metta experiment arena my_experiment --gpus 2" in help_text
        assert "metta notebook arena my_notebook" in help_text
        assert "# Create notebook without launching jobs" in help_text
        assert "# Equivalent to above" in help_text
    ...
    def test_compact_help_note(self):
        ...
        help_text_collapsed = format_help_with_defaults(
            TestConfig,
            prog_name="test_prog",
            collapse=True
        )
